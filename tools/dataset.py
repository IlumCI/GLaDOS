...
def emit_rust(path, train, test):
    ...
    lines.append("];")
    lines.append("")
    lines.append("/// How many of `SEED` are training examples.")
    lines.append(f"pub const SEED_TRAIN: usize = {len(train)};")
    lines.append("")
    lines.append("/// Where validation ends and the final test begins.")
    lines.append("///")
    lines.append("/// Three splits, not two, because the search selects among candidate")
    lines.append("/// configurations by score -- and selecting on a set is fitting it. With")
    lines.append("/// one held-out set the winner's score would be optimistic by however")
    lines.append("/// many candidates were tried. Validation is spent freely; the test")
    lines.append("/// slice is read once, at the end, to say what was actually achieved.")
    lines.append(f"pub const SEED_VAL_END: usize = {len(train) + len(test) // 2};")
    lines.append("")

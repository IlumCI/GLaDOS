...
def parse_weapons(src, states):
    """The nine weapons and the psprite states each one lives in.

    Emitted because the *rate of fire* is not a number anybody should choose:
    it is how long the attack chain takes to walk back to the ready state, so
    it belongs to the table and not to a constant somebody tuned.
    """
    idx = {n: i for i, n in enumerate(states)}
    m = re.search(r"pub const WEAPON_INFO: \[WeaponInfo; 9\] = \[(.*?)\n\];", src, re.S)
    if not m:
        raise SystemExit("no WEAPON_INFO array")
    rows = []
    for block in re.findall(r"WeaponInfo \{(.*?)\n    \},", m.group(1), re.S):
        def st(name):
            return idx[enum_ref(field(block, name))]

        rows.append(
            {
                "ammo": enum_ref(field(block, "ammo")),
                "up": st("upstate"),
                "down": st("downstate"),
                "ready": st("readystate"),
                "attack": st("atkstate"),
    ...
    if len(rows) != 9:
        raise SystemExit("%d weapons parsed, expected 9" % len(rows))
    return rows


def parse_rndtable(src):
    """DOOM's 256-byte random table.

    Generated with everything else rather than pasted, and it matters more here
    than for the states: this table *is* the game's randomness. Every copy of
    DOOM ever shipped draws from these bytes in this order, so a generator that
    invented its own -- or transcribed one badly -- would produce a machine
    that plays a subtly different game and could say so nowhere.
    """
    m = re.search(r"pub const RNDTABLE: \[i32; 256\] = \[(.*?)\];", src, re.S)
    if not m:
        raise SystemExit("no RNDTABLE")
    vals = [int(v) for v in re.findall(r"-?\d+", m.group(1))]
    if len(vals) != 256:
        raise SystemExit("%d entries in RNDTABLE, expected 256" % len(vals))
    if any(v < 0 or v > 255 for v in vals):
        raise SystemExit("RNDTABLE has a value outside a byte")
    return vals
...
def emit(sprites, states, kinds, flags, actions, weapons, rnd, origin):
    f = []
    w = f.append
    ...
    w("#![allow(dead_code)]")
    w("")
    w("/// DOOM's random table.")
    w("///")
    w("/// Not random at all: 256 bytes in a fixed order, walked by an index")
    w("/// that only ever advances. That is what makes a DOOM demo replay --")
    w("/// and it is what lets a run here be repeated exactly, which a real")
    w("/// generator would take away.")
    w("pub const RNDTABLE: [u8; 256] = [")
    for i in range(0, 256, 16):
        w("    " + " ".join("%d," % v for v in rnd[i:i + 16]))
    w("];")
    w("")
    ...
    w("}")
    w("")
    w("/// A weapon, and the psprite states it lives in.")
    w("///")
    w("/// `flash` is `S_NULL` for the fist and the chainsaw, which is the")
    w("/// table saying they do not light the room -- not a gap.")
    w("pub struct WeaponStates {")
    w("    pub up: u16,")
    w("    pub down: u16,")
    w("    pub ready: u16,")
    w("    pub attack: u16,")
    ...
    w("}")
    w("")
    w("/// The nine weapons, in the order the number keys select them.")
    w("pub const WEAPONS: [WeaponStates; 9] = [")
    for wp in weapons:
        w("    WeaponStates { up: %d, down: %d, ready: %d, attack: %d, flash: %d },"
          % (wp["up"], wp["down"], wp["ready"], wp["attack"], wp["flash"]))
    w("];")
    w("")
...
def main():
    ...
    # The weapons live in a third file again.
    defs_path = Path(args.checkout) / "gameplay" / "src" / "doom_def.rs"
    if not defs_path.exists():
        raise SystemExit("no %s" % defs_path)
    weapons = parse_weapons(defs_path.read_text(encoding="utf-8", errors="replace"), state_names)

    # And the random table, in a fourth file.
    math_path = Path(args.checkout) / "math" / "src" / "lib.rs"
    if not math_path.exists():
        raise SystemExit("no %s" % math_path)
    rnd = parse_rndtable(math_path.read_text(encoding="utf-8", errors="replace"))
    ...
    out = Path(args.out)
    out.write_text(
        emit(sprites, states, kinds, flags, actions, weapons, rnd,
             "room4doom gameplay/src/info.rs"),
        encoding="utf-8",
    )
    print(

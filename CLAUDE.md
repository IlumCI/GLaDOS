...
204 files, roughly 151,000 lines, of which about 13,000 are
`src/cpu/symbols.rs` -- a symbol table generated by `tools/symbols.py` from the
linker map and compiled back into the image it describes. **Two** things in the
kernel we did not write:
...
**It was three, and the third was `src/doom/`.** Nine thousand nine hundred
lines adapted from room4doom, brought over to answer one question: whether
software written somewhere else survives being carried into this kernel
without dissolving into it. It answered that -- the seam it forced,
`src/port/`, is the durable result and is still here -- and it was never
finished. So it is out of the build, out of the index and in `.gitignore`,
with the tree left on disk for anybody who wants to read it. Removing it took
the `doom`, `map` and `wad` shell verbs, one `diag` suite, the WAD that was
read at boot and the harness flag that staged one.

No WAD was ever in this repository and none ever will be.
...
in this tree -- so a knob under `src/gfx/` or `src/port/` would
...
the whole list.** Ported engines are measurements: `tools/xash.py` and
`tools/guest/spin.c` exist to find out whether a binary this kernel did not
compile can reach the screen, and `src/doom/` existed to ask the same thing
of source. What they establish is a fact about the kernel, and none of them
is a thing to install on somebody's laptop.
...
anybody's game data out of this repository. Neither is a thing to discover
after a
release is cut.
...
pointer motion**, a monotonic clock, and the bytes of a file. **Anything under
a ported tree may name `crate::port` and nothing else**, and that is checked
rather than intended:

```powershell
.	oolsenv\Scripts\python.exe tools\portcheck.py
```
...
**The seam is the part that was worth keeping.** It was built for the DOOM
port, on the reasoning that the point of a first port is to find out where the
boundary is -- a port that reaches into `gfx`, `kbd`, `sysbox` and `time`
wherever it needs them is not a port, it is a merge, and the second one starts
from nothing. DOOM is gone and the boundary it mapped is not: `edit` runs
through `port::with_screen`, `port bars` is the bounded full-screen fixture,
and `port::mouse` is the relative, unclamped pointer that `dev::mouse` cannot
be because it tracks a cursor that stops at the edge of the screen.

**`gfx::exclusive` is how a full-screen program keeps the screen.** Owning the
framebuffer is not enough: the taskbar readouts and the pointer are painted by
the compositor on its own schedule, so instead of contending for the paint
claim they stand down while the flag is set. `port::with_screen` sets it,
blanks the screen, and invalidates on the way out.
...
**A screenshot is taken when `drive.py` exits**, so for a program that hands
the screen back you get the desktop. To photograph the program itself, give it
a duration longer than the harness will wait and let the **timeout** be the
exit path.
...
the same flag `port::with_screen` sets for the editor, split into two
...
every full-screen program here needs.
...
beforehand are events nobody sees. It is the same answer `win keys` is, and it
borrows that spelling:
...
the exit path: `port bars 600000` with `--timeout 420` lands well inside the
run. Boot alone is around 370 s under WHPX with a real checkpoint, so a

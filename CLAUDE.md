...
itself. **No byte of id's data is in this repository or on the machine this was
built on**, so the renderer has never once been run against real DOOM art. It
should need no change for an IWAD or FreeDoom -- that is the whole reason the
reader was ported rather than written from the specification -- and "should" is
carrying real weight in that sentence, because nothing has tested it.

...
/// (`0x04 || X || Y`).
///
/// Generated by `tools/sign.py --keygen --out FILE`; the private half never
/// touches the machine being updated. Replace this to adopt a different signer
/// -- and note that doing so is itself a kernel change, which is the point.
///
/// **This is provisioned, and the paragraph that said otherwise outlived the
/// fact by months.** It read "unprovisioned on purpose, and this is the safe
/// state" long after a real point was pinned here, which is how a session went
/// looking for why the update channel could not publish, on the day it
/// published. The history it recorded is still true and still worth keeping: a
/// key was pinned during development and its private half was printed to a
/// terminal, a trust anchor whose private half has been seen is not one, and
/// zeroed is strictly better than compromised because `have_key` then refuses
/// everything and says so. What is no longer true is the present tense.
///
/// To rotate:
///
/// ```text
...
/// kernel change does. **Use `--out`**: without it the private half goes to
/// stdout, which is exactly how the last one died.
...
/// The public key this kernel will accept **verdicts** from.
///
/// A second anchor, and the separation is the whole of its value. A verdict is
/// a judgement about a change this machine proposed: it writes one line in
/// `/ai/godel/ledger.txt` and moves nothing else. An update is a kernel every
/// machine in the field will boot.
///
/// Those two deserve different keys because they are held in different places.
/// `UPDATE_KEY`'s private half signs releases, in a workflow that fires on a
/// tag or a branch push -- things that need write access to the repository.
/// The verdict half lives in `propose.yml`, which is reached by **any
/// allowlisted device** through the `proposal` function, because being
/// reachable that way is the entire point of the loop. Signing verdicts with
/// the update key would put the key that ships kernels into a workflow a
/// machine in the field can start.
///
/// So the blast radius of this one is: somebody who holds it can tell a
/// machine that a proposal it made was adopted or refused, and thereby write a
/// line in that machine's own lineage. That is worth protecting and it is not
/// worth the same protection.
///
/// **It cannot install anything.** `godel::read_verdict` is the only reader,
/// `update::verify` is the only path to a staged image, and the two check
/// different points -- so a verdict signature is not an update signature
/// however it is presented, and the claims below assert exactly that.
///
/// Rotated the same way and with the same consequence: a change here is a
/// kernel change.
pub const VERDICT_KEY: [u8; 65] = [
    0x04, 0x25, 0xf2, 0xca, 0x5b, 0xff, 0x8a, 0xb6,
    0x2d, 0x4a, 0xe5, 0x4c, 0xae, 0x3a, 0x3d, 0xc8,
    0xaf, 0xf5, 0x23, 0x1a, 0x3f, 0x96, 0xe7, 0x65,
    0x96, 0x98, 0x8e, 0x2e, 0x38, 0x2e, 0x37, 0xb3,
    0x70, 0x5c, 0x3a, 0x59, 0xec, 0xb5, 0xd6, 0x6e,
    0xa9, 0x29, 0x40, 0x3b, 0x7c, 0x42, 0x67, 0x19,
    0x17, 0xd0, 0xe7, 0xbe, 0x61, 0xe8, 0xa0, 0x76,
    0x68, 0xa1, 0x89, 0xde, 0xd4, 0x93, 0x18, 0x8a,
    0xbb,
];
...
pub fn have_key() -> bool {
    provisioned(&UPDATE_KEY)
}

/// Whether a verdict can be checked at all.
pub fn have_verdict_key() -> bool {
    provisioned(&VERDICT_KEY)
}

/// An uncompressed point that is not all zeroes.
///
/// One predicate for both anchors rather than two copies of the same
/// arithmetic: the day one of them gains a check the other does not is the day
/// a zeroed key is refused on one path and accepted on the other.
fn provisioned(key: &[u8; 65]) -> bool {
    key[0] == 0x04 && key[1..].iter().any(|b| *b != 0)
}
...
pub fn verify(image: &[u8], sig: &[u8]) -> Verdict {
    verify_with(&UPDATE_KEY, image, sig)
}

/// Check a detached signature over a **verdict**, against the other anchor.
///
/// Its own entry point rather than a flag on `verify`, because the two
/// questions must never be answerable by the same call with a different
/// argument somebody could get wrong. A caller that wants an update asks for
/// an update; a caller that wants a verdict asks for a verdict; and neither
/// name can be reached by passing the wrong key.
pub fn verify_verdict(text: &[u8], sig: &[u8]) -> Verdict {
    verify_with(&VERDICT_KEY, text, sig)
}

/// The shared arithmetic, taking the anchor as an argument.
///
/// One implementation, because two that are supposed to agree do not stay
/// agreeing -- and here a drift would mean one path checking a version field
/// the other does not, on the same bytes, with the difference invisible until
/// somebody forged a signature that one of them accepted.
fn verify_with(key: &[u8; 65], image: &[u8], sig: &[u8]) -> Verdict {
    if !provisioned(key) {
    ...
    if p256::verify(p256::Nist::P256, key, &digest, &sig[16..48], &sig[48..80]) {
        Verdict::Good
    } else {
        Verdict::Bad
    }
...
pub fn selftest() -> bool {
    ...
    // --- and the two anchors are two anchors ----------------------------
    //
    // **The separation is the point, so it is the thing to assert.** A verdict
    // is signed in `propose.yml`, which any allowlisted device can start
    // through the `proposal` function; an update is signed in `release.yml`,
    // which needs write access to the repository. If one key could stand in
    // for the other then the loop's own path would hold the key that ships
    // kernels, and the whole reason for a second point would be decoration.
    claim(
        "a verdict key is pinned, so a returning verdict can be checked at all",
        have_verdict_key(),
    );
    claim(
        "and it is not the update key",
        UPDATE_KEY != VERDICT_KEY,
    );
    // Both refuse the same malformed input, which is what says they share one
    // implementation rather than two that will drift.
    claim(
        "both anchors refuse an empty signature the same way",
        verify(image, &[]) == Verdict::Malformed
            && verify_verdict(image, &[]) == Verdict::Malformed,
    );
    claim(
        "and a future format the same way",
        verify(image, &wrong_version) == Verdict::Unsupported
            && verify_verdict(image, &wrong_version) == Verdict::Unsupported,
    );
    // The negative that matters cannot be written here without a private half,
    // and there is none on this machine -- which is the property the module
    // rests on. What *can* be checked is that the two calls consult different
    // points, so a signature good under one is being asked a different
    // question by the other. `tools/sign.py` drives the rest from outside:
    // a verdict signed with the update key is refused by `godel verdict`.
    claim(
        "the two verifications consult different points, so one cannot answer for the other",
        p256::verify(p256::Nist::P256, &UPDATE_KEY, &[0u8; 32], &zeros[16..48], &zeros[48..80])
            == p256::verify(
                p256::Nist::P256,
                &VERDICT_KEY,
                &[0u8; 32],

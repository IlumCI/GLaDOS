...
| `portfolio/index.html`, `portfolio/style.css` | Standalone page for euroswarms.eu or GitHub Pages. Hand-written HTML and CSS, no JavaScript, no external requests, no build step. Laid out as linux.org stood in July 2005: a fixed 860px frame on a cream field, a logotype block, a left rail of navigation buttons with a plate and a vitals table, and a white content well with a two-column opening, ruled section headers, square bullets and small data tables. Orange Box orange where that site used dark red. |
